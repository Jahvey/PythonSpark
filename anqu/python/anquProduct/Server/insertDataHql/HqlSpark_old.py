...
class HqlSpark():
	...
	def getAnalysisWords(self,appIds,genreIds):
		...
		if appWord and genreWord and thinkWord:
			appWord = appWord.unionAll(getAllWord)
	...
	def combine_data(self,words=None,result=None):
		...
		cluster_re = self.sc.parallelize(result)
		# print cluster_re.collect()
		re = words.map(list).zip(cluster_re).map(lambda p: Row(word=p[0][0], priority=int(p[0][1]),\
...
def main():
	...
	result = [1,2,1,1,2,1,2,2,1,3,3,3,1,2,1,1,2,1,2,2,1,3,3,3,1,2,1,1,2,1,2,2,1,3,3,3,1,2,1,1,2,1,2,2,1,3,3,3,1,2,1,1,2,1,2,2,1,3,3,3,1,2,1,1,2,1,2,2,1,3,3,3,1,2,1,1,2,1,2,2,1,3,3,3,\
	1,2,1,1,2,1,2,2,1,3,3,3]
